...
int main(int argc, char** argv)
{
	...
	do
	{
		...
		scanf("%d", &ladoMatriz);
		getchar(); //Con este getchar nos libramos del \n generado por el anterior scanf
		printf("Introduzca el alto de la matriz. \n");
		scanf("%d", &largoMatriz);
	} while (ladoMatriz*largoMatriz > prop.maxThreadsPerBlock); //Solo podemos introducir un tamaño de matriz que sea menor o igual al maximo de hilos por bloque
	...
	caracter = getchar();
	while (caracter != 'p')//El caracter de parada en el modo manual es la 'p'.
	{
