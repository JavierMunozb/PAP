  ...
  def comprobarDiagonalIzquierda(tablero: List[Char], index: Int, seleccion: List[Int] ,anterior: Int) : List[Int] = {
	   ...
	   if (anterior == 1) { //Anterior = 1 significa que el anterior movimiento fue horizontal
	    if (recorrerTablero(tablero, index) == recorrerTablero(tablero, index + 9)){ //El  elemento de la siguiente columna es igual 
	     ...
	     if (recorrerTablero(tablero, index) == recorrerTablero(tablero, index - 1)){ //El  elemento de la siguiente es igual, diagonal a izquierdas
	      if (seleccion.length + 1 >= 5){//Hemos encontrado 5 iguales juntas
		      index-1:: seleccion //Devolvemos la lista  
	      }
	...
	def comprobarDiagonales(tablero: List[Char], index: Int, seleccion: List[Int]) : List[Int] = {
	  ...
	  if (diagonalDerecha1.length >= 5){ //Empezamos la busqueda diagonal derecha por el inicio Horizontal
		     diagonalDerecha1
	      }
	  else if (diagonalDerecha2.length >= 5){ //Empezamos la busqueda diagonal derecha por el inicio Vertical
		     diagonalDerecha2
	      }
	  else if (diagonalIzquierda1.length >= 5){ //Empezamos la busqueda diagonal izquierda  por el inicio Horizontal
		     diagonalIzquierda1
	      }
	  else if (diagonalIzquierda2.length >= 5){ //Empezamos la busqueda diagonal izquierda por el inicio Horizontal
		     diagonalIzquierda2
	      }
	  else 
	...
	def comprobar5(tablero: List[Char]) : List[Int] = {
	  ...
	  if (comprobarHorizontal(tablero,1, Nil).length >= 5){ //Empezamos la busqueda por la horizontal
	   ...
	   else if (comprobarDiagonales(tablero,1, Nil).length >= 5){ //Hacemos busqueda Diafonal
